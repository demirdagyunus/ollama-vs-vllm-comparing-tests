...
@dataclass
class TestConfig:
    """Test configuration parameters for vLLM"""
...
class VLLMClient:
    ...
    def send_request(self, prompt: str) -> Tuple[bool, float, Optional[str], int, int, int]:
        """
        Send request to vLLM
        Returns: (success, latency, response_text, prompt_tokens, completion_tokens, total_tokens)
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return True, latency, response_text, int(prompt_tokens), completion_tokens, total_tokens
            else:
                logger.error(f"Request failed with status {response.status_code}: {response.text}")
                return False, latency, f"HTTP {response.status_code}: {response.text}", 0, 0, 0
            
        except requests.exceptions.Timeout:
            end_time = time.time()
            latency = end_time - start_time
            logger.error("Request timed out")
            return False, latency, "Request timeout", 0, 0, 0
        except Exception as e:
            end_time = time.time()
            latency = end_time - start_time
            logger.error(f"Request failed: {e}")
            return False, latency, str(e), 0, 0, 0
...
class PerformanceTester:
    """Main performance testing class for vLLM"""
    ...
    def run_warmup(self) -> None:
        ...
        while time.time() < warmup_end:
            ...
            success, _, _, _, _, _ = client.send_request(prompt)
            warmup_count += 1
            if success:
    ...
    def run_single_request(self, client: VLLMClient, request_id: int) -> RequestMetrics:
        """Run a single request and return metrics"""
        prompt = LangGraphAgent.get_random_prompt()
        timestamp = time.time()
        
        success, latency, response, prompt_tokens, completion_tokens, total_tokens = client.send_request(prompt)
    ...
    def run_all_tests(self) -> List[TestResult]:
        ...
        logger.info("=" * 80)
        logger.info("STARTING VLLM PERFORMANCE TESTS")
        logger.info("=" * 80)
    ...
    def generate_report(self, results: List[TestResult]) -> str:
        ...
        # Header
        report.append("=" * 100)
        report.append("VLLM PERFORMANCE TEST REPORT - QWEN3-4B")
        report.append("=" * 100)
        ...
        # vLLM Specific Recommendations
        report.append("VLLM SPECIFIC RECOMMENDATIONS")
        report.append("-" * 50)
        
        # Find optimal concurrency
        best_throughput_result = max(results, key=lambda r: r.throughput)
        best_latency_result = min(results, key=lambda r: r.avg_latency)
        
        report.append(f"• Optimal Throughput: {best_throughput_result.level} concurrent users "
                      f"({best_throughput_result.throughput:.2f} req/s)")
        report.append(f"• Best Latency: {best_latency_result.level} concurrent users "
                      f"({best_latency_result.avg_latency*1000:.1f}ms avg)")
        
        # Performance analysis
        if results[-1].error_rate > 5:
            report.append(f"• High error rate at maximum load ({results[-1].error_rate:.1f}%) - "
                          "consider adjusting vLLM max_parallel_loading_workers or max_model_len")
        
        if results[-1].avg_gpu_usage > 80:
            report.append("• High GPU usage detected - consider tensor parallelism or pipeline parallelism")
        
        report.append("• For vLLM optimization, consider:")
        report.append("  - Adjusting --max-model-len parameter")
        report.append("  - Using --tensor-parallel-size for multi-GPU setups")
        report.append("  - Enabling --enable-chunked-prefill for better throughput")
        report.append("  - Tuning --max-num-seqs based on your hardware")
    ...
    def save_results(self, results: List[TestResult], output_dir: str = "results"):
        ...
        report_file = os.path.join(output_dir, f"vllm_performance_report_{timestamp}.txt")
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(report)
        logger.info(f"Report saved to {report_file}")
        
        # Save JSON data for further analysis
        json_file = os.path.join(output_dir, f"vllm_performance_data_{timestamp}.json")
        json_data = {
...
def main():
    ...
    parser = argparse.ArgumentParser(description="vLLM Performance Test - Qwen3-4B")
    parser.add_argument("--model", default="Qwen/Qwen3-4B", help="Model name (Hugging Face format)")
    ...
    parser.add_argument("--api-key", default=None,
                        help="API key for vLLM (if required)")
    ...
    # Log configuration
    logger.info("vLLM Test Configuration:")
