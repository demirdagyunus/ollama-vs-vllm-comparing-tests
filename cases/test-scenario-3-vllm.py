...
class VLLMStreamingClient:
    """vLLM client optimized for streaming and conversational scenarios"""
    ...
    def send_streaming_request(self, prompt: str, conversation_history: List[str], 
        ...
        """
        Send streaming request to vLLM with comprehensive metrics
        ...
        try:
            ...
            # Prepare payload for vLLM streaming
            payload = {
                "model": self.config.model_name,
                "messages": messages,
                "max_tokens": self.config.max_tokens,
                "temperature": self.config.temperature,
                "stream": True if self.config.enable_streaming else False
            }
            ...
            if self.config.enable_streaming:
                ...
                if response.status_code == 200:
                        ...
                        if line:
                            ...
                            if line_decoded.startswith('data: '):
                                ...
                                try:
                                    ...
                                    logger.warning(f"Failed to parse streaming chunk: {data_part}")
                                    continue
                    ...
                    return False, time.time() - start_time, 0, f"HTTP {response.status_code}", 0, 0, 0, 0, 0, []
            ...
            # Estimate token usage (rough estimation for vLLM)
    ...
    def check_health(self) -> bool:
        ...
        try:
            logger.info("Checking health of vLLM streaming service")
            response = self.session.get(f"{self.config.base_url}/health", timeout=10)
            logger.info(f"Streaming health check successful, status: {response.status_code}")
...
class StreamingPerformanceTester:
    ...
    def check_service_health(self) -> bool:
        ...
        client = VLLMStreamingClient(self.config)
    ...
    def run_single_streaming_task(self, client: VLLMStreamingClient, task_id: str, 
    ...
    def run_all_tests(self) -> List[StreamingTestResult]:
        ...
        logger.info("=" * 80)
        logger.info("STARTING VLLM STREAMING & CONVERSATIONAL PERFORMANCE TESTS")
        logger.info("=" * 80)
        ...
        logger.info("✓ vLLM streaming service is healthy")
    ...
    def generate_report(self, results: List[StreamingTestResult]) -> str:
        ...
        # Header
        report.append("=" * 100)
        report.append("VLLM STREAMING & CONVERSATIONAL PERFORMANCE TEST REPORT - QWEN3-4B")
        report.append("=" * 100)
        ...
        # Recommendations
        report.append("VLLM STREAMING SPECIFIC RECOMMENDATIONS")
        report.append("-" * 50)
        ...
        if results:
            ...
            if any(r.avg_time_to_first_token > 2.0 for r in results):
                report.append("• High TTFT detected - consider optimizing --max-model-len or increasing workers")
            
            if any(r.avg_tokens_per_second < 10 for r in results):
                report.append("• Low token generation speed - consider --tensor-parallel-size or GPU optimization")
            
            if any(r.avg_response_coherence < 0.6 for r in results):
                report.append("• Low conversation coherence - consider adjusting temperature or context parameters")
        
        report.append("• For streaming optimization, consider:")
        report.append("  - Using --enable-chunked-prefill for better streaming performance")
        report.append("  - Adjusting --max-num-seqs based on your hardware capacity")
        report.append("  - Setting --tensor-parallel-size for multi-GPU setups")
        ...
        report.append("  - Use --disable-log-stats to reduce overhead during streaming")
    ...
    def save_results(self, results: List[StreamingTestResult], output_dir: str = "results/scenario-3"):
        ...
        report_file = os.path.join(output_dir, f"vllm_streaming_report_{timestamp}.txt")
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(report)
        logger.info(f"Streaming report saved to {report_file}")
        
        # Save JSON data
        json_file = os.path.join(output_dir, f"vllm_streaming_data_{timestamp}.json")
        json_data = {
...
def main():
    ...
    # Log configuration
    logger.info("vLLM Streaming Test Configuration:")
    ...
    try:
        ...
        if results:
            ...
            # Print summary
            print("\n" + "="*80)
            print("VLLM STREAMING TEST SUMMARY")
            print("="*80)
